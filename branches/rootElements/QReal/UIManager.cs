...
namespace QReal
{
    ...
    public class UIManager : DependencyObject
    {
        ...
        private static void OnSelectedGraphicInstancePropertyChanged(DependencyObject obj, DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                UIManager.Instance.InstancePropertiesSource = InstancesManager.Instance.InstancesContext.InstanceProperties.Where(item => item.LogicalInstance == UIManager.Instance.SelectedGraphicInstance.LogicalInstance);
                InstancesManager.Instance.SetCanvasRootElement(UIManager.Instance.SelectedGraphicInstance);
            }
            else
            {
                UIManager.Instance.InstancePropertiesSource = null;
            }
        }
